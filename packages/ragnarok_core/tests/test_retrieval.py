...
@pytest.mark.asyncio
async def test_rerank_texts():
    texts = ["你好", "你好吗", "瓦达西"]
    query = "你好"
    top_n = 2
    rerank_model = "Qwen/QwQ-32B"
    api_key = "sk-zxtyymbewqpbrrwtnwyissinlpwogrhlbeisiekaufdddiij"
    base_url = "https://api.siliconflow.cn/v1"
    result = await RetrievalComponent.rerank_texts(query, texts, top_n, rerank_model, api_key, base_url)
    print(result)
...
async def test_execute():
    ...
    texts = [
        ...
        """,
        """
        今天是一个难得的晴朗天气，阳光明媚，气温在25度左右，非常适合户外活动。
        微风轻拂，空气清新，能见度极佳。公园里的花草树木在阳光下显得格外生机勃勃，
        不少市民选择在这个时候出来散步、运动或者野餐。
        气象专家表示，这样的好天气将持续到明天，是进行户外活动的最佳时机。
        """,
        """
        天气预报显示明天将会有阵雨，气温会降至18度。气象台已经发布了黄色预警信号，提醒市民做好防范准备。建议出门携带雨伞，注意保暖，特别是老人和儿童要适当添加衣物。降雨可能会影响交通，建议提前规划出行路线，避开易积水路段。同时，也要注意防范雷电等强对流天气带来的影响。
        """,
        """
    ...
    await qdrant_client.delete_collection(embedding_model.value["name"])
    await qdrant_client.init_collection(name=embedding_model.value["name"], dim=embedding_model.value["dim"])
